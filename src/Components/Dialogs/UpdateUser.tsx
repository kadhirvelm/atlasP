...
class PureUpdateUser extends React.PureComponent<
  IDialogProps & IUpdateUserStoreProps & IUpdateUserDispatchProps
> {
  public state = {
        ...
        >
          <div className="user-fields-container">
            {Object.values(this.fields).map(value =>
              this.renderField(currentUser, value[0], value[1])
            )}
          </div>
  ...
  private renderTextfield(currentUser: IUser, key: string) {
    if (key === "password") {
      return (
        <PasswordField
          className={classNames("render-field-text", {
            error: this.checkIfInForceUpdate(key)
          })}
          placeHolder="Change password…"
          onChange={this.editUser(key)}
        />
      );
    }
    return (
  ...
  private editUser(key: string) {
    return (newValue: string) =>
      this.setState({
        currentUser: { ...this.state.currentUser, [key]: newValue }
      });
  }
  ...
  private containsErrors() {
    if (
      ...
      if (
        this.props.currentUser[changeField] ===
        this.state.currentUser[changeField]
      ) {
        this.maybeShowErrorToast();
        return true;
      }
  ...
  private maybeShowErrorToast() {
    if (this.props.forceUpdate !== undefined) {
      showToast(
        Intent.WARNING,
        `Before moving on, please update your ${this.props.forceUpdate.fields}.`
      );
    }
  }
}
